...
if [ $skip == false ];
	then
		...
		#check whether queryVCF exists
		if [ -e "$queryVCF_path" ];
	        then
				echo "queryVCF exists. First queryVCF provided: ${queryVCF_name}"
		...
		#if queryVCF does not exist
		else
			echo "No queryVCF provided. Exiting app."
		fi
